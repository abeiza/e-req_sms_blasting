			...
			$(function() {	
				$("#insert_item").click(function(){
					...
					$.ajax({
						url:"http://www.goc.co.id/index.php/backend/manage_career/update_data/",
						data:{
							id:$("#id").val(),
							title:$("#title").val(),
							dept:$("#dept").val(),
						...
						success:function(data){
							if(data.status == 'Sukses'){
								$.ajax({
									url:"<?php echo base_url();?>index.php/controller_posting/update_data/",
									data:{
										id:$("#id").val(),
										title:$("#title").val(),
										dept:$("#dept").val(),
			...
			<script>
				$(function() {	
					$(function(){
						$.ajax({
							url:"http://www.goc.co.id/index.php/backend/manage_career/get_items_grid_id/<?php echo $this->uri->segment(3);?>",
							cache:false,
							type: "POST",
							dataType: 'jsonp',
							jsonp:"mycallback",
							success:function(result){
								$.each(result, function(i, data){
									$('#id').val(data.Career_Id);
									$('#title').val(data.Career_Title);
									$('#dept').val(data.Career_Departmen);
									$('#position').val(data.Career_Position);
									$('#location').val(data.Career_Location);
									$('#edu').val(data.Career_Education);
									$('#experience').val(data.Career_Work_Experience);
									tinymce.get('desc').setContent(data.Career_Job_Desc);
									tinymce.get('req').setContent(data.Career_Requrement);
									$('#exp').val(data.Career_Job_Expired.substr(8,2)+'/'+data.Career_Job_Expired.substr(5,2)+'/'+data.Career_Job_Expired.substr(0,4));
									$('#co').val(data.Career_Qty);
									$('#status').val(data.Carrer_Status);
								});
							}
						});
					});						
			...
			<div style="width:82.5%;float:left;height:100%;background-color:#ffffff;overflow:auto;">
				<div style="width:100%;height:53px;border-bottom:1px solid #e7ecf1;padding-top:53px;display:flex;align-items:center;color:#666">
					<div style="width:50%;text-align:left;">
						<div style="padding:0px 20px;">Home <span class="fa fa-angle-right" style="margin:0px 5px;"></span> Posting  <span class="fa fa-angle-right" style="margin:0px 5px;"></span> Posting Edit</div>
					</div>
