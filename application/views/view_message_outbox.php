				...
				function delete_data(e){
					$.confirm({
						...
						confirm: function(){
							...
							$.ajax({
								url:"<?php echo base_url();?>index.php/controller_message/delete_outbox_record/",
								cache:false,
								data:{
									id:element
								},
					...
					<h1>Send Message</h1>
						<table>
							...
							<tbody>
								...
								<tr>
									<td colspan="3">Data Outbox is Not Available</td>
								</tr>
							<?php }else{ 
									foreach($outbox->result() as $db){
									?>
								<tr>
									<td><?php echo $db->ObjectID;?></td>
									<td><?php 
											$sender_number = explode(',',$db->no_destination);
											for($i=0;$i<count($sender_number);$i++){
												...
												if($name->num_rows() > 0){
													foreach($name->result() as $nm){
														echo "<div style='color:#fff;background-color:#26c6da !important;padding:3px;margin:3px;border-radius:3px;'>".$nm->contact_name=''?'':$nm->contact_name.'<br/>'."(".$sender_number[$i].")</div>";
													}
												}
											}
										?></td>
									<td><?php echo $db->text_msg;?></td>
									<td><?php echo $db->sender;?></td>
									<td><?php echo $db->send_date;?></td>
									<td>
										<?php 
										$no_dest = substr($db->no_destination,0,1)=='+'?substr($db->no_destination,1,50):$db->no_destination;
										echo form_open('controller_message/forward/'.$no_dest);?>
										<input type="hidden" name="no" value="<?php echo $db->no_destination;?>"/>
										<input type="hidden" name="text" value="<?php echo $db->text_msg;?>"/>
										<input type="hidden" name="date" value="<?php echo $db->send_date;?>"/>
										<button style='border:none;background-color:transparent;color:#666;font-size:12px;padding:3px 7px;text-decoration:none;border-radius:15px;'><span class="fa fa-mail-forward" style="margin-right:3px;"></span><br/>Forward</button>
										<?php echo form_close();?>
									</td>
									<td>
										<a href='<?php echo base_url().'index.php/controller_message/open/'; echo substr($db->no_destination,0,1)=='+'?substr($db->no_destination,1,50):$db->no_destination;?>' style='color:#666;font-size:12px;padding:3px 7px;text-decoration:none;border-radius:15px;'><span class="fa fa-tv" style="margin-right:3px;"></span><br/>Open</a>
									</td>
									<td>
