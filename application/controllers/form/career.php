<?php
	...
	class Career extends CI_Controller{
		...
		function proses_upload($id){
			...
			else
			{
				...
				else
				{
					...
					$data['resume_objectid'] = $this->model_app->getMaxKodelong('gocweb_resume_header', 'resume_objectid', 'APCR');
					...
					$insert_header = $this->model_app->get_insert('gocweb_resume_header',$data);
					...
					$insert_personal = $this->model_app->get_insert('gocweb_resume_personal',$data1);
					...
					$insert_family = $this->model_app->get_insert('gocweb_resume_family',$data2);
					...
					$insert_education = $this->model_app->get_insert('gocweb_resume_education',$data3);
					...
					$insert_experience = $this->model_app->get_insert('gocweb_resume_experience',$data4);
					...
					$insert_other = $this->model_app->get_insert('gocweb_resume_other',$data5);
					...
					if($insert_header && $insert_personal && $insert_family && $insert_education && $insert_experience && $insert_other){
						...
						Header("Location:".base_url()."index.php/form/career/psikotes/".$data['resume_kode_lamaran']);
					}else{
						//$this->session->set_flashdata("add_result",'<div style="margin:10px 20px;width:auto;background-color:#ffffbf;font-size:12px;border-radius:3px;color:red;padding:10px;"><span style="color:red; font-size:12px;"><i class="fa fa-exclamation-triangle" style="color:red;font-size:16px;margin-right:5px;"></i>Sorry your entry is fail.. Please try again.. </span></div>');
						Header("Location:".base_url()."index.php/form/career/form_applicant_personal/".$data['resume_jabatan']);
					}
				}
			}
		}
		...
		function proses_upload_test($id){
			...
			$insert_disc = $this->model_app->get_insert('gocweb_test_disc',$data_disc);
			...
			$insert_desc = $this->model_app->get_insert('gocweb_test_desc',$data_desc);
			
			if($insert_disc and $insert_desc){
				Header("Location:".base_url()."index.php/form/career/success_apply/");
			}else{
				$this->session->set_flashdata("add_result",'<div style="margin:10px 20px;width:auto;background-color:#ffffbf;font-size:12px;border-radius:3px;color:red;padding:10px;"><span style="color:red; font-size:12px;"><i class="fa fa-exclamation-triangle" style="color:red;font-size:16px;margin-right:5px;"></i>Sorry your entry is fail.. Please try again.. </span></div>');
				Header("Location:".base_url()."index.php/form/career/form_applicant_personal/".$data['resume_jabatan']);
			}
